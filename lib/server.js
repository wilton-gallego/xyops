...
class ServerManager {
	...
	validateServer(server) {
		// check server hostname and group
		var self = this;
		...
		if (server.group) {
			server.group = this.storage.normalizeKey( server.group );
		}
		...
		if (!server.group) {
			this.autoAssignServerGroup(server);
			
			// set autoGroup flag so group can be reassigned later if hostname matches change, groups resort, etc.
		...
		// validate group
		var group_def = Tools.findObject( group_defs, { id: server.group });
		if (!group_def) {
			var msg = "Unknown group: " + server.group + " (sent from server: " + server.hostname + ")";
			this.logTransaction('warning', msg, server);
			server.err = msg;
			return false; // failure
		}
	...
	autoAssignServerGroup(server) {
		// automatically assign group based on hostname match, and group sort order
		var group_defs = this.groups;
		var old_group = server.group;
		var lowest_sort_order = 999999;
		
		for (var idx = 0, len = group_defs.length; idx < len; idx++) {
			var group_def = group_defs[idx];
			if (server.hostname.match(group_def.hostname_match) && (group_def.sort_order < lowest_sort_order)) {
				server.group = group_def.id;
				lowest_sort_order = group_def.sort_order;
			}
		}
		
		return (server.group != old_group);
	}
	
	updateServerGroups() {
		// update all server group assignments, called after groups change
		for (var server_id in this.servers) {
			var server = this.servers[server_id];
			if (server.autoGroup) {
				if (this.autoAssignServerGroup(server)) {
					// server group has changed -- notify server about this
					this.doServerBroadcast( server_id, 'update', { group: server.group } );
				} // changed
			} // autoGroup
		} // foreach server
	}
